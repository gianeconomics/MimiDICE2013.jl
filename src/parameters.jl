...
function getdice2013gamsparameters(filename)
    ...
    p[:b11]         = getparams(f, "B22:B22", :single, sheet, 1)       # Carbon cycle transition matrix atmosphere to atmosphere
    p[:b12]         = getparams(f, "B20:B20", :single, sheet, 1)       # Carbon cycle transition matrix atmosphere to shallow ocean
    p[:b21]         = getparams(f, "B23:B23", :single, sheet, 1)       # Carbon cycle transition matrix biosphere/shallow oceans to atmosphere
    p[:b22]         = getparams(f, "B24:B24", :single, sheet, 1)       # Carbon cycle transition matrix shallow ocean to shallow oceans
    p[:b23]         = getparams(f, "B21:B21", :single, sheet, 1)       # Carbon cycle transition matrix shallow to deep ocean
    p[:b32]         = getparams(f, "B25:B25", :single, sheet, 1)       # Carbon cycle transition matrix deep ocean to shallow ocean
    p[:b33]         = getparams(f, "B26:B26", :single, sheet, 1)       # Carbon cycle transition matrix deep ocean to deep oceans
    p[:c1]          = getparams(f, "B37:B37", :single, sheet, 1)       # Speed of adjustment parameter for atmospheric temperature (per 5 years)
    p[:c3]          = getparams(f, "B38:B38", :single, sheet, 1)       # Coefficient of heat loss from atmosphere to oceans
    p[:c4]          = getparams(f, "B39:B39", :single, sheet, 1)       # Coefficient of heat gain by deep oceans
    p[:cca0]        = getparams(f, "B14:B14", :single, sheet, 1)       # Initial cumulative industrial emissions
    p[:cost1]       = getparams(f, "B47:BI47", :all, sheet, T)         # Abatement cost function coefficient
    p[:damadj]      = 1.0                                              # Adjustment exponent in damage function (not used in GAMS version).
    p[:dk]          = getparams(f, "B8:B8", :single, sheet, 1)         # Depreciation rate on capital (per year)
    p[:elasmu]      = getparams(f, "B55:BI55", :single, sheet, 1)      # Elasticity of MU of consumption
    p[:eqmat]       = getparams(f, "B31:B31", :single, sheet, 1)       # Equilibirum concentration of CO2 in atmosphere (GTC)
    p[:etree]       = getparams(f, "B13:BI13", :all, sheet, T)         # Carbon emissions form land use change (GTCO2 per year)
    p[:expcost2]    = getparams(f, "B49:B49", :single, sheet, 1)       # Exponent of control cost function
    p[:fco22x]      = getparams(f, "B30:B30", :single, sheet, 1)       # Forcings of equilibrium CO2 doubling (Wm-2)
    p[:forcoth]     = getparams(f, "B29:BI29", :all, sheet, T)         # Exogenous forcing for other greenhouse gases
    p[:gama]        = getparams(f, "B7:B7", :single, sheet, 1)         # Capitail Share
    p[:k0]          = getparams(f, "B9:B9", :single, sheet, 1)         # Initial capital
    p[:l]           = getparams(f, "B6:BI6", :all, sheet, T)           # Level of population and labor (millions)
    p[:mat0]        = getparams(f, "B17:B17", :single, sheet, 1)       # Initial Concentration in atmosphere at end 2007 (GtC)
    p[:MIU]         = getparams(f, "B48:BI48", :all, sheet, T)         # Optimized emission control rate results from DICE2013 (base case)
    p[:ml0]         = getparams(f, "B19:B19", :single, sheet, 1)       # Initial Concentration in deep oceans 2008 (GtC)
    p[:mu0]         = getparams(f, "B18:B18", :single, sheet, 1)       # Initial Concentration in biosphere/shallow oceans 2008 (GtC)
    p[:partfract]   = getparams(f, "B50:BI50", :all, sheet, T)         # Fraction of emissions in control regime
    p[:pbacktime]   = getparams(f, "B51:BI51", :all, sheet, T)         # Backstop price ($1000 per ton CO2)
    p[:rr]          = getparams(f, "B56:BI56", :all, sheet, T)         # Social Time Preference Factor
    p[:S]           = getparams(f, "B52:BI52", :all, sheet, T)         # Optimized savings rate (fraction of gross output) results from DICE2013 (base case)
    p[:scale1]      = getparams(f, "B57:B57", :single, sheet, 1)       # Multiplicative scaling coefficient
    p[:scale2]      = getparams(f, "B58:B58", :single, sheet, 1)       # Additive scaling coefficient
    p[:sigma]       = getparams(f, "B12:BI12", :all, sheet, T)         # (industrial, MTCO2/$1000    2000 US$)
    p[:t2xco2]      = getparams(f, "B34:B34", :single, sheet, 1)       # Equilibrium temp impact (oC per doubling CO2)
    p[:tatm0]       = getparams(f, "B35:B35", :single, sheet, 1)       # Initial atmospheric temp change 2008-2011 (C from 1900)
    p[:tocean0]     = getparams(f, "B36:B36", :single, sheet, 1)       # Initial temperature of deep oceans (deg C aboce 1900)
    p[:usedamadj]   = false
